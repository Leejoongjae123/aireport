...
import ConsultingSearchFilter from "./ConsultingSearchFilter";
...
export default function ConsultingContent({ initialData }: ConsultingContentProps) {
  const [data, setData] = useState<ConsultingRequestResponse>(initialData);
  ...
  const fetchData = async (page: number, limit: number) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/expert-consulting?${params}`);
      if (response.ok) {
  ...
  return (
    <div className="flex flex-col items-center gap-8 p-11 bg-white min-h-screen font-['Pretendard']">
      ...
      <div className="w-full">
        <h1 className="text-2xl font-bold text-[#2A2A2A]">전문가 컨설팅 요청</h1>
      </div>

      {/* Filter Section */}
      <ConsultingSearchFilter onSearch={handleSearch} />

      {/* Status Summary Cards */}
      <ConsultingStatsCards stats={data.stats} />
